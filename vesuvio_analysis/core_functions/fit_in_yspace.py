...
def dataXBining(ws, xp):
    ...
    assert np.min(xp[:-1] - xp[1:]) == np.max(xp[:-1] - xp[1:]), (
...
def avgArr(dataYO):
    ...
    assert np.all(np.all(dataYO == 0, axis=0) == (meanY == 0)), (
...
def weightedAvgArr(dataYO, dataEO):
    ...
    # Run some tests
    assert dataYO.shape == dataEO.shape, (
    ...
    assert np.all(np.isnan(dataYO) == np.isnan(dataEO)), (
...
def symArr(dataYO):
    ...
    (
        np.testing.assert_array_equal(dataYS, np.flip(dataYS, axis=1)),
...
def weightedSymArr(dataYO, dataEO):
    ...
    (
        np.testing.assert_array_equal(dataES, np.flip(dataES, axis=1)),
...
def selectModelAndPars(modelFlag):
    ...
    if modelFlag == "SINGLE_GAUSSIAN":
        ...
        def model(x, A, x0, sigma):
            return (
                A / (2 * np.pi) ** 0.5 / sigma * np.exp(-((x - x0) ** 2) / 2 / sigma**2)
            )
    ...
    assert all(isinstance(item, str) for item in sharedPars), (
...
def createFitParametersTableWorkspace(
    ...
    # --- FIX START: Helper to safely convert None to NaN ---
    def safe_float(val):
        """Converts None to np.nan to prevent Mantid table crashes."""
    ...
    for p, v, e, mae, mme in zip(
        ...
        # --- FIX START: Sanitize the Minos error values
        ## Old code:
    ...
    for minimizer in ["Levenberg-Marquardt", "Simplex"]:
        if yFitIC.fitModel == "SINGLE_GAUSSIAN":
            ...
            *(1.+c6/384*(64*((x-x0)/sqrt(2)/sigma1)^6 - 480*((x-x0)/sqrt(2)/sigma1)^4 + 720*((x-x0)/sqrt(2)/sigma1)^2 - 120)),
            ...
            | (yFitIC.fitModel == "MULTIVARIATE_GAUSSIAN")
        ):
            return
        else:
...
def extractData(ws, wsRes, ic):
    ...
    assert len(instrPars) == len(dataY), (
...
def avgGroupsOverCols(dataX, dataY, dataE, dataRes, idxList):
    ...
    for i, idxs in enumerate(idxList):
        ...
        assert np.all(groupX[0] == np.mean(groupX, axis=0)), (
