...
def loadRawAndEmptyWsFromUserPath(ic):
    ...
    assert (type(ic.scaleRaw) == float) | (type(ic.scaleRaw) == int), (
    ...
    if ic.subEmptyFromRaw:
        ...
        assert (type(ic.scaleEmpty) == float) | (type(ic.scaleEmpty) == int), (
...
def cropAndMaskWorkspace(ic, ws):
    ...
    assert ic.firstSpec >= wsFirstSpec, (
...
def histToPointData(dataY, dataX, dataE):
    ...
    assert np.min(histWidths) == np.max(histWidths), (
...
def createNcpWorkspaces(ncpForEachMass, ncpTotal, ws, ic):
    ...
    assert ncpTotal.shape == dataX.shape, (
...
def extractMeans(wsName, IC):
    ...
    meanWidths, stdWidths, meanIntensityRatios, stdIntensityRatios = (
        calculateMeansAndStds(widths, intensities, IC)
    )

    assert len(widths) == IC.noOfMasses, (
...
def filterWidthsAndIntensities(widthsIn, intensitiesIn, IC):
    ...
    if np.all(np.isnan(betterIntensities)) & IC.runningPreliminary:
        assert IC.noOfMSIterations == 0, (
            "Calculation of mean intensities failed, cannot proceed with MS correction. Try to run again with noOfMSIterations=0."
        )
    ...
    if len(betterWidths) > 1:
        assert np.nanmax(betterWidths) != np.nanmin(betterWidths), (
            f"All fitered widths have the same value: {np.nanmin(betterWidths)}"
        )
        assert np.nanmax(betterIntensities) != np.nanmin(betterIntensities), (
...
def calculateNcpSpec(
    ...
    FSE = (
        -numericalThirdDerivative(ySpacesForEachMass, JOfY) * widths**4 / deltaQ * 0.72
    )
...
def calcGaussianResolution(masses, v0, E0, delta_E, delta_Q, resolutionPars, instrPars):
    ...
    assert masses.shape == (masses.size, 1), (
    ...
    dW2 = (
        dWdE1**2 * dE1**2 + dWdTOF**2 * dTOF**2 + dWdL1**2 * dL1**2 + dWdL0**2 * dL0**2
    )
...
def createWorkspacesForGammaCorrection(ic, meanWidths, meanIntensityRatios, wsNCPM):
    ...
    SetInstrumentParameter(
        inputWS, ParameterName="hwhm_lorentz", ParameterType="Number", Value="24.0"
    )
    SetInstrumentParameter(
        inputWS, ParameterName="sigma_gauss", ParameterType="Number", Value="73.0"
    )
    ...
    background, corrected = VesuvioCalculateGammaBackground(
        InputWorkspace=inputWS, ComptonFunction=profiles
    )

    RenameWorkspace(
        InputWorkspace=background, OutputWorkspace=inputWS + "_Gamma_Background"
    )
    Scale(
