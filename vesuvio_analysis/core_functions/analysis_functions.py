...
def _print_optimizer_agreement_summary() -> None:
    ...
    if not gate_pass and _fit_comparison_log:
        ...
        # Per-spectrum Top-10 sorted by max relative difference
        sorted_specs = sorted(
            _fit_comparison_log,
            key=lambda d: max(d.get("chi2_rel_diff", 0), d.get("max_par_diff", 0)),
            reverse=True,
        )[:10]
        print("  Top-10 Spectra by Max Relative Difference:")
        for rank, entry in enumerate(sorted_specs, 1):
            par_names_e = entry.get("par_names", [])
            par_rel_e = entry.get("par_rel_diff", np.array([]))
            if len(par_rel_e) > 0 and len(par_names_e) > 0:
                n_e = min(len(par_rel_e), len(par_names_e))
                worst_idx = int(np.argmax(par_rel_e[:n_e]))
                worst_param = par_names_e[worst_idx]
                worst_diff = float(par_rel_e[worst_idx])
            else:
                worst_param, worst_diff = "N/A", float("nan")
            max_diff_pct = max(
                entry.get("chi2_rel_diff", 0),
                entry.get("max_par_diff", 0),
            ) * 100
            print(
                f"    {rank:2d}. Spec {int(entry['specNo']):4d}  "
                f"max_diff={max_diff_pct:.1f}%  "
                f"worst_param={worst_param}  ({worst_diff * 100:.1f}%)"
            )
...
def _plot_optimizer_comparison(ic: Any) -> None:
    ...
    # Individual per-spectrum PDFs with Fisher Score metadata are generated by
    # plot_optimizer_divergence_spectra(), called from _runPreFitStatistics
    # (Phase 6) once Fisher scores are available.
    # Store a snapshot here so the phase-6 call can access the correct bank data.
    ic._fit_comparison_snapshot = list(_fit_comparison_log)


def plot_optimizer_divergence_spectra(
    ic: Any,
    debug_mode: bool = False,
    fisher_scores_map: Optional[Dict[float, float]] = None,
) -> None:
    """Generate per-spectrum optimizer divergence PDFs for failing spectra.

    Saves individual comparison PDFs to ``ic.figSavePath / "optimizer_divergence"``.
    Each PDF includes Transparency First metadata: Spectrum ID, scattering
    angle, and Fisher Score (when provided via *fisher_scores_map*).

    This function is called from Phase 6 (``_runPreFitStatistics``) after
    Fisher scores are available.  It reads the fit-comparison snapshot stored
    on *ic* by ``_plot_optimizer_comparison`` during the fitting phase.

    Args:
        ic: Initial-conditions object with ``figSavePath``, ``name``,
            ``InstrParsPath``, ``firstSpec``, ``lastSpec``.
        debug_mode: If ``True``, saves plots for ALL spectra including those
            within threshold.  If ``False`` (default), only spectra that
            exceed ``_AGREEMENT_THRESHOLD``.
        fisher_scores_map: Optional mapping of ``spec_no`` (float) to Fisher
            discriminant score (float) to annotate plot titles.
    """
    # Use the per-IC snapshot captured during fitNcpToWorkspace; fall back to
    # the module-level log for backward compatibility.
    log: list = getattr(ic, "_fit_comparison_snapshot", None) or _fit_comparison_log
    if not log or ic.runningSampleWS:
        return

    if debug_mode:
        entries_to_plot = list(log)
    else:
        entries_to_plot = [
            d for d in log
            if d.get("max_par_diff", 0) > _AGREEMENT_THRESHOLD
            or d.get("chi2_rel_diff", 0) > _AGREEMENT_THRESHOLD
        ]

    if not entries_to_plot:
        return
    ...
    def _lookup_angle(spec_no: float) -> float:
        if _instr_pars is None:
            return float("nan")
        row = np.where(np.isclose(_instr_pars[:, 0], spec_no))[0]
        return float(_instr_pars[row[0], 2]) if len(row) == 1 else float("nan")

    # Create save subdirectory
    div_dir = ic.figSavePath
    try:
        div_dir = ic.figSavePath / "optimizer_divergence"
        div_dir.mkdir(parents=True, exist_ok=True)
    except Exception as exc:
        logger.warning("Could not create optimizer_divergence directory: %s", exc)
        div_dir = ic.figSavePath
    ...
    for entry in entries_to_plot:
        ...
        fisher_score = (fisher_scores_map or {}).get(float(spec_no), float("nan"))
        par_names = entry.get("par_names", [])
        ...
        fisher_str = f"{fisher_score:.3f}" if np.isfinite(fisher_score) else "N/A"
        title = (
            f"Fit Comparison \u2014 Spec {spec_no}  "
            f"(\u03b8 = {angle:.2f}\u00b0  |  Fisher Score: {fisher_str})"
        )

        fig_spec, ax_spec = plt.subplots(figsize=(8, 3 + 0.4 * n_pars))
        ax_spec.axis("off")
        ax_spec.set_title(title, fontsize=11, fontweight="bold")
        ...
        plt.tight_layout()
        try:
            spec_fileName = f"{ic.name}_Fit_Comparison_Spec_{spec_no}.pdf"
            fig_spec.savefig(div_dir / spec_fileName, bbox_inches="tight", pad_inches=0.1)
        except Exception as exc:
            logger.warning(
                "Failed to save divergence plot for spec %d: %s", spec_no, exc,
            )
        plt.close(fig_spec)

    print(
        f"[OptimizerDivergence] Generated {len(entries_to_plot)} "
        f"plot(s) in {div_dir}"
    )
